...
export function buildPersistentDestructionLayer(plan, seconds, options = {}) {
  ...
  for (const shot of shots.filter((item) => item.hitType === 'destroy' || item.authorityType === 'destroy')) { const actor = actors.get(shot.targetId); const t = Number(shot.impactTime ?? shot.t) || 0; if (t > seconds + 1e-9 || seconds - t < .95 || !shot.impactPositionAtImpact) continue; wrecks.push({ id: `wreck_${shot.targetId}`, sourceActorId: shot.targetId, sourceType: actor?.type || null, side: actor?.side || null, visualClass: normalizeVisualUnitClass(actor), x: shot.impactPositionAtImpact.x, y: shot.impactPositionAtImpact.y, angle: Number(shot.targetFacingAtImpact ?? 0), wreckType: wreckType(actor), smoke: true, persistent: true, createdAt: t, visualFootprint: actor?.footprint || null, seed: shot.seed ?? deterministicUnit(plan?.source?.seed ?? 0, shot.id) }); }
  for (const event of destroyEvents(plan, shots)) { if (event.t > seconds || seconds - event.t < .95 || !event.point) continue; const actor = actors.get(event.targetId); wrecks.push({ id: `wreck_${event.targetId}`, sourceActorId: event.targetId, sourceType: actor?.type || null, side: actor?.side || null, visualClass: normalizeVisualUnitClass(actor), x: event.point.x, y: event.point.y, angle: Number(actor?.facing) || 0, wreckType: wreckType(actor), smoke: true, persistent: true, createdAt: event.t, seed: deterministicUnit(plan?.source?.seed ?? 0, event.id) }); }
