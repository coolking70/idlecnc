...
function drawActor(context, actor, plan, options) {
  ...
  const point = worldPoint(actor.visualCenter, plan.layout.bounds); const spec = options.drawSpecByActor?.get(actor.id) || actor.drawSpec || {}; const scale = Number(spec.visualScaleBoost) || 1; const entrySize = spec.worldSize || (actor.type === 'mbt' ? { width: 60, height: 36 } : actor.category === 'infantry' ? { width: 22, height: 28 } : { width: 44, height: 22 }); const assetWidth = Number(entrySize.width || 30) * VIEW_WIDTH / Math.max(1, Number(plan.layout.bounds.width) || 1200) * scale; const assetHeight = Number(entrySize.height || 20) * VIEW_HEIGHT / Math.max(1, Number(plan.layout.bounds.height) || 700) * scale; const assetDrawn = spec.assetMode !== 'procedural' && spec.assetId && options.assetRuntime?.draw(context, spec.assetId, point.x, point.y, assetWidth, assetHeight, actor.facing || 0);
  if (!assetDrawn) { context.save(); context.translate(point.x, point.y); context.scale(scale, scale); context.translate(-point.x, -point.y); if (actor.category === 'infantry' || actor.type === 'at_infantry') drawInfantry(context, actor, plan.layout.bounds); else drawVehicle(context, actor, plan.layout.bounds); context.restore(); }
  else if (spec.assetMode === 'hybrid' && actor.type === 'mbt') drawHybridTankOverlay(context, actor, point, plan.layout.bounds);
  const radius = actor.type === 'mbt' ? 35 * scale : actor.category === 'infantry' ? 25 * scale : 28 * scale;
...
export class UniversalBattleRenderer {
  ...
  render(activeBattle) {
    ...
    const returning = activeBattle?.presentationPhase === 'returning'; const plan = this.presentation.plan; const duration = Math.max(.001, Number(activeBattle?.duration || plan.timeline.sourceDuration || 1)); const seconds = returning ? plan.timeline.duration : Math.max(0, Number(activeBattle?.elapsed || 0) / duration * plan.timeline.duration);
  ...
  getActorScreenMetricsAt(seconds) {
    ...
    const state = this.presentation.renderState.atTime(Number(seconds) || 0); const viewport = this.resize(); const specs = buildProductionDrawSpecs({ actors: state.actors, wrecks: state.wrecks, environment: state.environment, camera: state.camera, options: { battlefieldBounds: this.presentation.plan.layout?.bounds || { width: 1200, height: 700 }, viewport } });
    return { metricSpace: 'final_css_pixels', viewport: { ...viewport }, camera: { ...state.camera }, actors: specs.actorSpecs.map((spec) => ({ actorId: spec.actorId, side: spec.faction, type: spec.type, minimumScreenFootprint: spec.minimumScreenFootprint, rawScreenFootprint: spec.rawScreenFootprint, visualScaleBoost: spec.visualScaleBoost, screenFootprint: spec.screenFootprint, viewportScale: spec.viewportScale, metricSpace: spec.metricSpace })) };
  }
