...
export class UniversalBattleRenderer {
  constructor(canvas, options = {}) {
    this.canvas = canvas; this.context = canvas?.getContext?.('2d'); this.options = normalizeDebugOverlayOptions({ showHud: true, ...options }); this.options.showHud = options.showHud !== false; this.assetRuntime = createAssetRuntime(); this.presentation = null; this.lastState = null; this.lastTime = 0; this.activeBattle = null; this.cameraMode = 'overview'; this.autoCamera = true; this.cameraOverride = null; this.viewport = null; this.pointer = null;
    this._bindPointer();
  ...
  resize() { if (!this.canvas) return null; this.viewport = measurePresentationViewport(this.canvas); const { width, height, dpr } = this.viewport; if (this.canvas.width !== Math.floor(width * dpr) || this.canvas.height !== Math.floor(height * dpr)) { this.canvas.width = Math.floor(width * dpr); this.canvas.height = Math.floor(height * dpr); } return this.viewport; }
  _bindPointer() {
  ...
  reset() { this.presentation = null; this.activeBattle = null; this.lastState = null; this.lastTime = 0; this.cameraOverride = null; this.autoCamera = true; this.pointer = null; this.canvas?.classList?.remove('is-camera-dragging'); }
  render(activeBattle) {
