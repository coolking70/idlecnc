...
function drawActor(context, actor, plan, options) {
  ...
  let assetDrawn = false; let turretDrawn = false;
  if (spec.assetMode !== 'procedural' && spec.assetId && spec.sourceRect && options.assetRuntime) {
    if (visualClass === 'mbt') {
      assetDrawn = options.assetRuntime.drawFrame(context, spec.assetId, spec.sourceRect, point.x, point.y, drawRect.width, drawRect.height, 0);
      const turretId = spec.componentAssetIds?.turretAssetId; const turretRect = spec.turretSourceRect || spec.sourceRect;
      if (assetDrawn && turretId && turretRect) turretDrawn = options.assetRuntime.drawFrame(context, turretId, turretRect, point.x, point.y, drawRect.width, drawRect.height, 0);
    } else assetDrawn = options.assetRuntime.drawFrame(context, spec.assetId, spec.sourceRect, point.x, point.y, drawRect.width, drawRect.height, 0);
  }
  if (!assetDrawn) {
    ...
    context.restore();
  } else if (visualClass === 'mbt' && !turretDrawn) drawHybridTankOverlay(context, actor, point, plan.layout.bounds);
...
function drawEffects(context, effects, bounds) {
  for (const effect of effects || []) {
    ...
    if ((effect.start || effect.sourcePosition) && (effect.end || effect.targetPosition) && ['rocket_trail', 'repair_beam', 'tracer', 'cannon', 'welding'].includes(effect.kind)) { const start = worldPoint(effect.start || effect.sourcePosition, bounds); const end = worldPoint(effect.end || effect.targetPosition, bounds); context.strokeStyle = effect.kind === 'repair_beam' || effect.kind === 'welding' ? '#d6eff6' : effect.kind === 'cannon' ? '#f5d27b' : '#ef9a67'; context.lineWidth = effect.kind === 'repair_beam' ? 2.5 : effect.kind === 'cannon' ? 3 : 2; context.setLineDash(effect.kind === 'repair_beam' ? [5, 4] : []); context.beginPath(); context.moveTo(start.x, start.y); context.lineTo(end.x, end.y); context.stroke(); context.setLineDash([]); if (effect.kind === 'rocket_trail') { context.fillStyle = '#fff0b0'; context.beginPath(); context.arc(end.x, end.y, 3.4, 0, TAU); context.fill(); context.globalAlpha = alpha * .45; context.fillStyle = '#9d8570'; context.beginPath(); context.arc(start.x, start.y, 5 + (1 - alpha) * 4, 0, TAU); context.fill(); } }
    if (['impact', 'muzzle', 'hit_spark', 'muzzle_flash', 'impact_spark', 'damage_spark'].includes(effect.kind)) { const muzzle = effect.kind === 'muzzle_flash'; const size = Number(effect.size) || 12; context.strokeStyle = muzzle ? '#e9d889' : '#f7d489'; context.lineWidth = Number(effect.tracerWidth) || (effect.weaponKind === 'cannon' ? 4 : 2); context.beginPath(); context.arc(target.x, target.y, size * (muzzle ? .72 : .65), 0, TAU); context.stroke(); context.fillStyle = '#fff2ad'; context.beginPath(); context.arc(target.x, target.y, (Number(effect.impactScale) || 1) * (effect.weaponKind === 'cannon' ? 5 : 3), 0, TAU); context.fill(); if (effect.kind === 'damage_spark') for (let index = 0; index < 6; index += 1) { const angle = index * 1.05 + Number(effect.damageEventId?.length || 0) * .03; context.beginPath(); context.moveTo(target.x, target.y); context.lineTo(target.x + Math.cos(angle) * size, target.y + Math.sin(angle) * size); context.stroke(); } }
    if (['destruction', 'explosion', 'heavy_impact', 'rocket_impact', 'cannon_impact', 'destroy_flash'].includes(effect.kind)) { const size = Number(effect.size) || 26; context.fillStyle = effect.kind === 'cannon_impact' || effect.kind === 'heavy_impact' ? '#efb65e' : '#ef9e50'; context.beginPath(); context.arc(target.x, target.y, size * (.72 + alpha * .28), 0, TAU); context.fill(); context.fillStyle = '#fff0a8'; context.beginPath(); context.arc(target.x, target.y, size * .32, 0, TAU); context.fill(); if (['rocket_impact', 'cannon_impact', 'destruction'].includes(effect.kind)) { context.strokeStyle = '#e9c26c'; context.lineWidth = 2; context.beginPath(); context.arc(target.x, target.y, size * 1.15 * (1 - alpha * .18), 0, TAU); context.stroke(); } }
    if (effect.kind === 'muzzle_flash') { context.fillStyle = effect.weaponKind === 'cannon' || effect.weaponFamily === 'tank_cannon' ? '#fff0a8' : effect.weaponFamily === 'anti_armor' ? '#f2b66e' : '#f7cf74'; const size = Number(effect.size || 12); const facing = Number(effect.sourceFacingAtFire) || 0; context.translate(target.x, target.y); context.rotate(facing); context.beginPath(); if (effect.muzzleShape === 'small_flash' || effect.muzzleShape === 'rapid_flash') context.arc(0, 0, size * .55, 0, TAU); else if (effect.muzzleShape === 'repair_spark') context.arc(0, 0, size * .35, 0, TAU); else { context.moveTo(0, 0); context.lineTo(size, -size * .35); context.lineTo(size * .45, 0); context.lineTo(size, size * .35); context.closePath(); } context.fill(); }
    if (['damage_smoke', 'destroy_smoke', 'wreck_smoke'].includes(effect.kind)) { context.fillStyle = effect.kind === 'damage_smoke' ? '#a39b81' : '#6f766e'; context.globalAlpha = alpha * .62; context.beginPath(); context.arc(target.x, target.y, (Number(effect.size) || 13) * (1.1 - alpha * .2), 0, TAU); context.fill(); }
    if (effect.kind === 'wreck_fire') { context.fillStyle = '#d77043'; context.beginPath(); context.moveTo(target.x, target.y + 7); context.quadraticCurveTo(target.x - 8, target.y - 2, target.x, target.y - 15); context.quadraticCurveTo(target.x + 9, target.y - 3, target.x + 5, target.y + 7); context.closePath(); context.fill(); context.fillStyle = '#ffe39a'; context.beginPath(); context.arc(target.x, target.y - 3, 4, 0, TAU); context.fill(); }
    if (['welding', 'repair_spark'].includes(effect.kind)) { const end = effect.end || effect.targetPosition; if (end) { const spark = worldPoint(end, bounds); context.strokeStyle = '#d9f5ff'; context.lineWidth = 2; for (let index = 0; index < 6; index += 1) { const angle = index * 1.05; context.beginPath(); context.moveTo(spark.x, spark.y); context.lineTo(spark.x + Math.cos(angle) * Number(effect.size || 16) * alpha, spark.y + Math.sin(angle) * Number(effect.size || 16) * alpha); context.stroke(); } } }
...
function drawPresentationTransition(context, transition, width, height) {
  if (!transition || Number(transition.alpha) <= .001 || transition.phase === 'battle' || transition.phase === 'completed') return;
  const alpha = Math.max(0, Math.min(.32, Number(transition.alpha) || 0));
  context.save(); context.globalAlpha = alpha; context.fillStyle = transition.phase === 'intro' ? '#10221e' : ['victory_outro', 'pyrrhic_outro'].includes(transition.kind) ? '#26412d' : '#251d1a'; context.fillRect(0, 0, width, height);
  context.globalAlpha = Math.min(.9, alpha + .22); context.fillStyle = transition.phase === 'intro' ? '#c2d7af' : '#e1c887'; context.font = '700 13px sans-serif'; context.textAlign = 'center'; context.fillText(transition.kind === 'battle_intro' ? '战斗展开' : transition.kind === 'withdraw_outro' ? '有序撤离' : transition.kind === 'victory_outro' ? '目标完成' : '战斗结束', width / 2, height - 42); context.textAlign = 'start'; context.restore();
}
...
export class UniversalBattleRenderer {
  ...
  render(activeBattle, _dtReal = 0, exactSeconds = null) {
    ...
    // geometry here, so screen metrics and actual drawImage use the same frame.
