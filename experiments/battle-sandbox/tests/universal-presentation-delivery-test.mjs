...
assert.equal(coverage.missionResultMatrix.totalCells, 30); assert.equal(coverage.missionResultMatrix.coveredCells, 26); assert.equal(coverage.missionResultMatrix.unobservedCells.length, 4);
assert.ok(!coverageText(coverage).includes('undefined'), 'coverage dashboard must not display undefined fields'); assert.match(coverageText(coverage), /mission×result 26\/30/);
