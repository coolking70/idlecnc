...
export function launchManagedVerifierProcess({ cwd, root = cwd, port = 0, node = process.execPath, env = process.env } = {}) {
  ...
  const child = spawn(node, ['--input-type=module', '-e', source], { cwd, env, detached: process.platform !== 'win32', stdio: ['ignore', 'pipe', 'pipe'] });
  ...
  const timer = setTimeout(() => {
    ...
    readySettled = true;
    rejectReady(new Error('managed verifier server start timeout'));
  }, 10000);
  ...
  child.once('error', (error) => {
    state.error = error;
    state.exited = true;
    if (!readySettled) { readySettled = true; clearTimeout(timer); rejectReady(error); }
    resolveExit(state);
  });
  child.once('exit', (code, signal) => {
    ...
    state.code = code;
    state.signal = signal;
    if (!readySettled && !handle.port) { readySettled = true; clearTimeout(timer); rejectReady(new Error(`managed verifier server exited ${code || signal}`)); }
    resolveExit(state);
  });
  handle.ready = ready;
  return handle;
...
export async function terminateManagedVerifierProcess(handle, options = {}) {
  ...
  if (!state.exited) { state.killedBySigkill = true; signalProcess(handle, 'SIGKILL'); state = { ...(await waitForExit(handle, killTimeoutMs)), killedBySigkill: true }; }
  return { ...state, cleaned: Boolean(state.exited) };
}
