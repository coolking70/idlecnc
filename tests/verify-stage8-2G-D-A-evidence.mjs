...
for (const frame of machineFrames) {
  ...
  for (const expected of expectedRows) { const actual = actualRows.find((row) => row.actorId === expected.actorId); if (!actual) fail(`D-A missing browser actor ${frame.file}`); const expectedComparable = geometryComparable(expected); const actualComparable = geometryComparable(legacyBrowserRow(actual, expectedComparable)); if (frame.fallbackExpected && expected.actorId === 'unit_stage8g-c1-mining-0') { expectedComparable.actualDrawPath = 'procedural-fallback'; } if (canonicalEvidenceString(actualComparable) !== canonicalEvidenceString(expectedComparable)) fail(`D-A browser renderer rows ${frame.file}`); }
