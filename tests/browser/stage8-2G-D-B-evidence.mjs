  ...
  try {
    ...
    for (const scene of machine.scenes) {
      ...
      for (const frame of scene.frames) {
        ...
        if (frame.semantic === 'repair-vehicle-action') { const repair = evidence.state.actors.find((actor) => actor.type === 'repair_vehicle'); if (repair?.drawSpec?.animation !== 'repair' || repair.visualStatus !== 'repairing') throw new Error(`D-B repair state missing ${frame.file}`); }
        ...
        capturedFrames.push({ ...frame, selectedActorId: evidence.selection?.selectedActorId || null, captureTimeMs: actualTimeMs, timestampDeltaMs: actualTimeMs - frame.timeMs, viewport: await viewport(), stateSignature: evidence.c1StateSignature, screenMetrics, hudContract: evidence.hudContract, selection: evidence.selection, assetRuntime: { allReady: assetRuntime?.allReady === true, assets: assetRuntime?.assets || [] }, imageSha256, screenshot: { path: path.relative(root, pngPath), sha256: imageSha256 }, browserStateSnapshot: evidence.state });
      }
