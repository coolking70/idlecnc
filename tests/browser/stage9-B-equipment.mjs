...
async function main() {
  ...
  try {
    ...
    // Stage 10-P-B moved these controls into the shared Command Inspector.
    // Reaching one stays a real two-step DOM interaction: click the tile to open
    // the Inspector, then click the Inspector action. The action buttons mirror
    // their payload as data-* attributes, so a specific instance stays addressable.
    ...
    await tab('units'); await waitSelector('[data-command-id="unit-instance:stage9-b-browser-unit"]', 'unit tile');
    await clickInspectorAction('[data-command-id="unit-instance:stage9-b-browser-unit"]', '[data-inspector-action="equip-equipment"][data-equipment-instance-id="equipment-starter-2"]', { action: 'equip-equipment', unitId: 'stage9-b-browser-unit', equipmentInstanceId: 'equipment-starter-2' });
    ...
    await clickInspectorAction('[data-command-id="unit-instance:stage9-b-browser-unit"]', '[data-inspector-action="unequip-equipment"][data-equipment-instance-id="equipment-starter-2"]', { action: 'unequip-equipment', unitId: 'stage9-b-browser-unit', equipmentInstanceId: 'equipment-starter-2' });
    ...
    await clickInspectorAction('[data-command-id^="formation:f"]', '[data-inspector-action="add-unit"]', { action: 'add-unit' }); await waitUntil((s) => s.formations?.[0]?.unitIds?.includes('stage9-b-browser-unit'), 'unit added to formation');
    ...
    await tab('units'); await clickInspectorAction('[data-command-id="unit-instance:stage9-b-browser-unit"]', '[data-inspector-action="unequip-equipment"][data-equipment-instance-id="equipment-starter-2"]', { action: 'running-equipment-change-attempt', rejectedBy: 'battle_lock' }, { allowDisabled: true }); await capture(2, { phase: 'running_equipment_change_attempt', sessionId: running.activeBattle.battleSessionId });
    const runningReload = await reloadPage('running_battle'); await tab('units'); await clickInspectorAction('[data-command-id="unit-instance:stage9-b-browser-unit"]', '[data-inspector-action="unequip-equipment"][data-equipment-instance-id="equipment-starter-2"]', { action: 'running-equipment-change-attempt-after-reload', rejectedBy: 'battle_lock' }, { allowDisabled: true }); await capture(3, { phase: 'running_after_real_reload', realReload: runningReload, sessionId: running.activeBattle.battleSessionId });
    await cdp.evaluate(call('tickBattle', 999)); await waitUntil((s) => s.activeBattle?.settled === true, 'result panel'); await tab('units'); await clickInspectorAction('[data-command-id="unit-instance:stage9-b-browser-unit"]', '[data-inspector-action="unequip-equipment"][data-equipment-instance-id="equipment-starter-2"]', { action: 'result-equipment-change-attempt', rejectedBy: 'battle_lock' }, { allowDisabled: true }); await capture(4, { phase: 'result_equipment_change_attempt' });
    const resultReload = await reloadPage('result'); await tab('units'); await clickInspectorAction('[data-command-id="unit-instance:stage9-b-browser-unit"]', '[data-inspector-action="unequip-equipment"][data-equipment-instance-id="equipment-starter-2"]', { action: 'result-equipment-change-attempt-after-reload', rejectedBy: 'battle_lock' }, { allowDisabled: true }); await capture(5, { phase: 'result_after_real_reload', realReload: resultReload });
    ...
    await tab('reports'); await clickInspectorAction('[data-command-id^="report:"]', '[data-inspector-action="replay-report"]', { action: 'replay-report' }); await waitUntil((s) => s.activeBattle?.replayReadOnly === true, 'replay started'); await tab('units'); await clickInspectorAction('[data-command-id="unit-instance:stage9-b-browser-unit"]', '[data-inspector-action="unequip-equipment"][data-equipment-instance-id="equipment-starter-2"]', { action: 'replay-equipment-change-attempt', rejectedBy: 'replay_read_only' }, { allowDisabled: true }); await capture(6, { phase: 'replay_equipment_change_attempt' });
    const replayReload = await reloadPage('replay'); await tab('units'); await clickInspectorAction('[data-command-id="unit-instance:stage9-b-browser-unit"]', '[data-inspector-action="unequip-equipment"][data-equipment-instance-id="equipment-starter-2"]', { action: 'replay-equipment-change-attempt-after-reload', rejectedBy: 'replay_read_only' }, { allowDisabled: true }); await capture(7, { phase: 'replay_after_real_reload', realReload: replayReload });
