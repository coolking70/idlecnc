...
async function main() {
  ...
  try {
    ...
    await tab('formations'); await click('[data-action="create-formation"]', { action: 'create-formation' }); await waitUntil((s) => s.formations?.length === 1, 'first formation'); await click('[data-action="add-unit"]', { action: 'add-unit' }); await waitUntil((s) => s.formations?.[0]?.unitIds?.includes('stage9-d-browser-unit'), 'first unit assigned');
    ...
    await tab('theater'); await click('[data-action="select-theater"][data-theater="scrap_mine"]', { action: 'select-theater', theaterId: 'scrap_mine' }); await click('[data-action="select-strategy"][data-strategy="cautious"]', { action: 'select-strategy', strategyId: 'cautious' }); await click('[data-action="launch-battle"]', { action: 'open-deployment-review' }); await waitSelector('[data-action="confirm-dispatch"]', 'dispatch confirm'); await click('[data-action="confirm-dispatch"]', { action: 'confirm-dispatch' }); await waitUntil((s) => Boolean(s.activeBattle?.battleSessionId), 'first battle running');
    await cdp.evaluate(call('tickBattle', 99999)); await waitUntil((s) => s.activeBattle?.settled === true, 'first settlement'); await tab('theater'); await waitSelector('[data-action="claim-battle-salvage"]', 'salvage claim'); await capture(0, { phase: 'result_salvage_available' });
    ...
    await click('[data-action="claim-battle-salvage"]', { action: 'claim-battle-salvage' }); await waitUntil((s) => Object.keys(s.equipment?.salvageClaims || {}).length === 1, 'salvage claimed'); await capture(2, { phase: 'salvage_claimed' });
    ...
    await tab('formations'); await click('[data-action="create-formation"]', { action: 'create-formation', fixture: 'no_drop' }); await waitUntil((s) => s.formations?.length === 1, 'no-drop formation'); await click('[data-action="add-unit"][data-unit-id="stage9-d-browser-no-drop-unit"]', { action: 'add-unit', fixture: 'no_drop' }); await waitUntil((s) => s.formations?.[0]?.unitIds?.includes('stage9-d-browser-no-drop-unit'), 'no-drop unit assigned');
    ...
    await tab('theater'); await click('[data-action="select-theater"][data-theater="scrap_mine"]', { action: 'select-theater', theaterId: 'scrap_mine', fixture: 'no_drop' }); await click('[data-action="select-operation"][data-operation-id="salvage_run"]', { action: 'select-operation', operationId: 'salvage_run', fixture: 'no_drop' }); await click('[data-action="select-strategy"][data-strategy="cautious"]', { action: 'select-strategy', strategyId: 'cautious', fixture: 'no_drop' }); await click('[data-action="launch-battle"]', { action: 'open-deployment-review', fixture: 'no_drop' }); await waitSelector('[data-action="confirm-dispatch"]', 'no-drop dispatch confirm'); await click('[data-action="confirm-dispatch"]', { action: 'confirm-dispatch', fixture: 'no_drop' }); await waitUntil((s) => Boolean(s.activeBattle?.battleSessionId), 'no-drop battle running'); await cdp.evaluate(call('tickBattle', 99999)); await waitUntil((s) => s.activeBattle?.settled === true, 'no-drop settlement'); await tab('theater'); await waitUntil((s) => s.activeBattle?.settled === true, 'no-drop result'); await capture(7, { phase: 'no_drop_result' });
