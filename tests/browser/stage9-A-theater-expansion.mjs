...
async function main() {
  ...
  try {
    ...
    await tab('construction'); await click('[data-action="build"][data-building-type="barracks"]', { action: 'build', buildingType: 'barracks' }); await cdp.evaluate(call('setSpeed', 4)); await waitUntil((s) => s.buildings?.some((b) => b.type === 'barracks' && b.status === 'operational') && !s.construction?.current, 'barracks operational'); await cdp.evaluate(call('setSpeed', 0)); await waitEnabled('[data-action="build"][data-building-type="armor_factory"]', 'armor factory build');
    await click('[data-action="build"][data-building-type="armor_factory"]', { action: 'build', buildingType: 'armor_factory' }); await cdp.evaluate(call('setSpeed', 4)); await waitUntil((s) => s.buildings?.some((b) => b.type === 'armor_factory' && b.status === 'operational') && !s.construction?.current, 'armor factory operational'); await cdp.evaluate(call('setSpeed', 0)); await capture(1, { phase: 'armor_factory_operational' });
    ...
    for (let i = 0; i < 3; i += 1) { const before = (await state()).units.filter((u) => u.type === 'mbt').length; await waitEnabled('[data-action="produce"][data-unit-type="mbt"]', `MBT production control ${i + 1}`, 120000); await click('[data-action="produce"][data-unit-type="mbt"]', { action: 'produce', unitType: 'mbt', ordinal: i + 1 }); await cdp.evaluate(call('setSpeed', 4)); await waitUntil((s) => s.units.filter((u) => u.type === 'mbt').length > before, `MBT production ${i + 1}`, 120000); await cdp.evaluate(call('setSpeed', 0)); if (i < 2) { await cdp.evaluate(call('setSpeed', 4)); await waitUntil((s) => s.resources?.supply >= 220 && s.resources?.alloy >= 300, `MBT production resources ${i + 2}`, 120000); await cdp.evaluate(call('setSpeed', 0)); } }
    ...
    while ((await state()).formations[0].unitIds.length < 3) { const before = (await state()).formations[0].unitIds.length; await click('[data-action="add-unit"]', { action: 'add-unit' }); await waitUntil((s) => s.formations?.[0]?.unitIds?.length > before, 'MBT added'); }
    ...
    await tab('theater'); await click('[data-action="select-theater"][data-theater="river_crossing"]', { action: 'select-theater', theaterId: 'river_crossing' }); await click('[data-action="select-strategy"][data-strategy="cautious"]', { action: 'select-strategy', strategyId: 'cautious' }); await waitUntil((s) => s.formations?.some((f) => f.status === 'idle' && f.unitIds.length === 3), 'river campaign eligibility'); await capture(4, { phase: 'river_crossing_eligibility', prerequisiteStateSeeded: true });
    ...
    await click('[data-action="select-theater"][data-theater="river_crossing"]', { action: 'select-theater', theaterId: 'river_crossing' }); await click('[data-action="select-strategy"][data-strategy="cautious"]', { action: 'select-strategy', strategyId: 'cautious' }); await click('[data-action="launch-battle"]', { action: 'open-deployment-review' }); await waitSelector('[data-action="confirm-dispatch"]', 'campaign confirm');
    ...
    await click('[data-action="view-report"]', { action: 'view-report' }); await waitSelector('[data-action="replay-report"]', 'report view'); await capture(11, { phase: 'formal_report_view' });
    ...
    await click('[data-action="select-operation"][data-operation-id="river_ferry"]', { action: 'select-operation', operationId: 'river_ferry' }); await capture(12, { phase: 'repeat_operation_eligibility', missionKind: 'operation' });
