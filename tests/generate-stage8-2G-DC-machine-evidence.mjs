...
const definitions = [
  ...
  { sceneId: 'stage8g-dc-withdraw', report: fixture('campaign-withdraw.json'), result: 'withdraw', frames: [['d-c-11-scout-fire.png', 'scout-fire', 'default'], ['d-c-14-defeat-withdraw-outro.png', 'withdraw-outro', 'narrow']] },
...
for (const definition of definitions) {
  ...
  const frames = definition.frames.map(([file, semantic, viewportKind]) => {
    ...
    const effectBindings = (state.effects || []).map((effect) => ({ id: effect.id, kind: effect.kind, x: effect.x, y: effect.y, shotId: effect.shotId || null, actorId: effect.actorId || null, targetActorId: effect.targetActorId || effect.target || null, weaponFamily: effect.weaponFamily || null, sourceEventId: effect.sourceEventId || null, impactEventId: effect.impactEventId || null, damageEventId: effect.damageEventId || null, destroyEventId: effect.destroyEventId || null, repairEventId: effect.repairEventId || null, repairSourceActorId: effect.repairSourceActorId || null, repairTargetActorId: effect.repairTargetActorId || null, authoritySource: effect.authoritySource || null })).sort((left, right) => String(left.id).localeCompare(String(right.id)));
    return { file, semantic, sceneId: definition.sceneId, seed: definition.report.seed, viewportKind, visualTimeSeconds: resolution.visualTimeSeconds, timeMs: resolution.timeMs, semanticCheck: true, sceneHash: presentation.plan.planFingerprint || presentation.plan.source?.reportFingerprint || null, stateSignature: resolution.stateSignature, semanticResolution: resolution, matchedActorIds: resolution.matchedActorIds, matchedShotIds: resolution.matchedShotIds, matchedEffectIds: effectBindings.map((effect) => effect.id), effectBindings, effectKinds: resolution.effectKinds, effectInventory: resolution.effectInventory, cameraFeedback: resolution.cameraFeedback, audioCues: resolution.audioCues, transitions: resolution.transitions };
  });
