...
export function verifyStage9EEvidence(candidate, { checkFiles = false } = {}) {
  const failures = [];
  ...
  if (checkFiles) {
    ['stage9_e_machine_evidence.json', 'stage9_e_browser_capture_manifest.json', 'stage9_e_machine_evidence.json'].forEach((name) => { if (!fs.existsSync(path.join(root, name))) fail(failures, `files.${name}`, 'exists', 'missing'); });
  }
