...
const previous = fs.existsSync('stage8_2g_dc1_developer_selfcheck.json') ? readJson('stage8_2g_dc1_developer_selfcheck.json') : null;
const output = { stage: '8.2G-D-C.1', version: 1, requiredArtifacts: required, missing, strongVerifier: verdict, performanceP95Ms: performance.scenes.map((row) => ({ sceneId: row.sceneId, p95Ms: row.p95Ms })), tamperRejectionCount: tamper.rejectionCount, browserCaptureCount: browser.browser.captureCount, uniqueImageHashes: browser.browser.uniqueImageHashes, formalReportImportedAsReadOnly: true, formalRepairAuthorityModified: bundle.authority.formalRepairAuthorityModified === true, ...(previous?.ci ? { ci: previous.ci } : {}), ...(previous?.readyToCloseStage8_2G_D_C === true ? { readyToCloseStage8_2G_D_C: true } : {}), passed: true };
