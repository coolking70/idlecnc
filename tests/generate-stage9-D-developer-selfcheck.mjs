...
const forbidden = ['js/theater.js', 'js/production-battle-session.js', 'js/save-diff.js', 'js/battle.js', 'js/battle-presentation/universal/', 'tests/lib/'];
const diffNames = execFileSync('git', ['diff', '--name-only'], { cwd: root, encoding: 'utf8' }).split(/\r?\n/).filter(Boolean);
const answers = [
  { id: 1, question: 'remote commit', answer: 'pending until final push', verified: false },
  { id: 2, question: 'clean clone', answer: 'pending until final final-head run', verified: false },
  ...
  { id: 4, question: 'settlement write-set', answer: 'claim is post-settlement and equipment-only; no settlement module change', verified: bundle.authority?.formalSettlementEquipmentUnchanged === true && !diffNames.includes('js/theater.js') },
  ...
  { id: 12, question: 'tamper', answer: { cases: tamper.caseCount, rejected: tamper.rejectionCount, passedFlagOnlyCases: tamper.passedFlagOnlyCases }, verified: tamper.caseCount >= 80 && tamper.rejectionCount === tamper.caseCount && tamper.passedFlagOnlyCases === 0 },
  { id: 13, question: 'authority files', answer: 'frozen authority source files and tests/lib are unchanged', verified: diffNames.every((file) => !forbidden.some((prefix) => file === prefix || file.startsWith(prefix))) },
...
const output = {
  ...
  passed: answers.slice(2).every((row) => row.verified) && strong.passed === true && tamper.passedFlagOnlyCases === 0 && requiredEvidence.every((file) => fs.existsSync(path.join(root, file)))
};
